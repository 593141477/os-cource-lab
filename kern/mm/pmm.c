...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if (!(*pdep & PTE_U)) {              // (2) check if entry is not present
        if(create) {                  // (3) check if creating is needed, then alloc page for page table
            ...
            uintptr_t pa = page2kva(pg); // (5) get linear address of page
            memset(pa, 0, PGSIZE);              // (6) clear page content using memset
            *pdep = page2pa(pg)|PTE_P|PTE_W|PTE_U;      // (7) set page directory entry's permission
        }else {
            return NULL;
        }
    }
