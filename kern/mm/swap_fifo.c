...
_fifo_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     assert(victim != head);
     list_del(victim);  //(1)  unlink the  earliest arrival page in front of pra_list_head qeueue
     *ptr_page = le2page(victim, pra_page_link); //(2)  set the addr of addr of this page to ptr_page
     return 0;
}
